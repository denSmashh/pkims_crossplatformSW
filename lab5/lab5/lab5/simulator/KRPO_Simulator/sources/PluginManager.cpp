...
bool PluginManager::ListPluginsOnStartup() {
  ...
  do {
    if (hFind == nullptr) {
      ...
      hFind = FindFirstFile(L".\\simulator\\KRPO_Simulator\\bin\\modules\\*.dll", &FindFileData);
    ...
    pi.fileName = std::string(".\\simulator\\KRPO_Simulator\\bin\\modules\\") + cfilename_str;
    ...
    if (NULL == hDll) {
      ...
      std::cout << "\tОшибка LoadLibrary: " << error << std::endl;
      continue;
    }
